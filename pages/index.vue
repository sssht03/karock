<template>
  <div>
    ...
    <input type="text" v-model="giftPurpose" placeholder="What is the gift for?" />
...
async function upload() {
  if (file.value && giftPurpose.value && fromName.value && toName.value && message.value) {
    const documentId = 'your-document-id'; // これは適切なロジックで取得するか、ユーザーが提供するID
    await $uploadData(file.value, giftPurpose.value, fromName.value, toName.value, message.value, documentId);
  } else {
